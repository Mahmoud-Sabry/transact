...
class Charts extends Component {
  static navigationOptions = {
    ...
    drawerIcon: ({ tintColor }) => (
      <Image
        source={require("../../Images/cashier.png")}
        style={[styles.icon]}
      />
    )
  };
  ...
  onSelectCategory() {
    const FEEDBACK_CATEGORIES = ["Revenue", "Profit", "Product"];
    ActionSheetIOS.showActionSheetWithOptions(
      {
        options: FEEDBACK_CATEGORIES
      },
      buttonIndex => {
        if (FEEDBACK_CATEGORIES[buttonIndex] == "Profit")
          this.setState({
            chart: FEEDBACK_CATEGORIES[buttonIndex],
            list: this.props.topSellingCashiersByGrossProfit,
            cashType: "EGP"
          });
        else if (FEEDBACK_CATEGORIES[buttonIndex] == "Product")
          this.setState({
            chart: FEEDBACK_CATEGORIES[buttonIndex],
            list: this.props.topSellingCashiersBySoldProducts,
            cashType: "items"
    ...
    return (
      <View>
        <Header style={styles.HeaderStyle}>
          ...
          <Body style={{ paddingLeft: Platform.OS === "ios" ? 0 : width / 8 }}>
